...
import { insertShootReferenceSchema } from '../../../shared/schema';
import { z } from 'zod';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (req.method === 'GET') {
      ...
      try {
        const data = insertShootReferenceSchema.parse({ ...req.body, shootId });
        ...
        if (error instanceof z.ZodError) {
          ...
          res.status(500).json({ error: error.message || 'Failed to add reference' });
        }
      }
    } else {
