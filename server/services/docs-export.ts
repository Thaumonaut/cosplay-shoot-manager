...
export async function createShootDocument(shoot: ShootWithDetails, providedDocsClient?: any): Promise<{ docId: string; docUrl: string }> {
  ...
  const footerText = `\n\nGenerated by CosPlans on ${format(new Date(), 'MMMM d, yyyy \'at\' h:mm a')}`;
  requests.push({
    insertText: {
      location: { index: currentIndex },
      text: footerText,
