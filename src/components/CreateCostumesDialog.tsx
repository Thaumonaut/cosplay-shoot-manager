...
import type { CostumeProgress } from "@/lib/shared-schema";
...
export function CreateCostumesDialog({
  ...
  useEffect(() => {
    if (editItem) {
      setCharacterName(editItem.character || "");
      setSeriesName(editItem.series || "");
      setStatus("planning"); // CostumeProgress doesn't have status, using default
      setNotes(editItem.notes || "");
      setTodos([]); // CostumeProgress doesn't have todos
      setImagePreview(""); // CostumeProgress doesn't have imageUrl
      setImageFile(null);
    }
  }, [editItem]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    formData.append("name", characterName.trim()); // Using characterName as name
    formData.append("character", characterName.trim());
    if (seriesName.trim()) {
      formData.append("series", seriesName.trim());
    }
    if (notes.trim()) {
      formData.append("notes", notes.trim());
    }
    // Remove status, todos, and image - not in CostumeProgress schema
