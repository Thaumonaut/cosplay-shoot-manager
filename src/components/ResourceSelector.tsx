...
export function ResourceSelector<T extends Resource>({
    ...
    <div className="flex items-center gap-3 flex-1 min-w-0">
      {resource.imageUrl && (
        <div className="h-10 w-10 rounded-md overflow-hidden bg-muted shrink-0 relative">
          <Image src={resource.imageUrl} alt={resource.name} fill className="object-cover" />
        </div>
      )}
      <div className="flex-1 min-w-0">
        <p className="font-medium truncate">{resource.name}</p>
  ...
  const renderCostumeDetails = (resource: CostumeResource) => (
    <div className="flex items-center gap-3 flex-1 min-w-0">
      {resource.imageUrl && (
        <div className="h-10 w-10 rounded-md overflow-hidden bg-muted shrink-0 relative">
          <Image src={resource.imageUrl} alt={resource.characterName} fill className="object-cover" />
        </div>
      )}
      <div className="flex-1 min-w-0">
